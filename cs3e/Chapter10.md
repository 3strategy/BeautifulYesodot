...
![2d array](/assets/img/Chapter10/2darrays.png)

{: .box-success}
סדר הוראה: אני מעדיף ללמד לאחר מספר שיעורים של עצמים כספירלה על מערכים ולולאות.

![Gilder gun](/assets/img/Chapter10/Gospers_glider_gun.gif)

## מבוא למערך דו-מימדי (מטריצה)

דוגמה קלאסית להמחשת המושג היא תבנית ביצים, שבה הביצים מסודרות בשתי שורות ומספר עמודות.

![eggs](/assets/img/Chapter10/eggs2.png)
...
[תרגול אינטראקטיבי ראשוני (קל) למערך דו-מימדי](/cs3e/Ex/Chapter10BidiInteractive)
...
1. `for(i=0; i<5; i++) Console.Write(m[i,i]);` -> ידפיס את **האלכסון הראשי**: 1, 7, 13, 19, 25.

[תרגול אינטראקטיבי נוסף: לולאות במערך דו-מימדי](/cs3e/Ex/Chapter10BidiInteractiveDiag)
...
## חומר למחיקה - נא להתעלם

{: .table-heh}

| סוג | הגדרה | תיאור |
| --- | --- | --- |
| תכונות | string nafasdfme | שם הדלי |
| סתם טקסט ארוך | int capacity | קיבולת הדלי |

<details markdown="1"><summary>טעימה לסיום: משחק החיים (Game of Life)</summary>

**הערה:** שאלה יחסית קשה המתאימה לתלמידים שלומדים מערך דו-ממדי בסוף החומר וכבר שולטים בחומר.

משחק החיים הוא משחק סימולציה המתאר את מחזור החיים של יצורים (או תאים) חיים.
הסימולציה מתרחשת במטריצה שבה כל תא מייצג אתר מחיה - בכל אתר אחד משני מצבים:
I. "יש חיים" - אתר מאוכלס ביצור - מסומן באמצעות המספר 1.
II. "אין חיים" - אתר ריק - מסומן באמצעות המספר 0.

חוקי הגנטיקה במשחק החיים:
1. כל יצור שיש לו פחות משני שכנים, מת מבדידות.
2. כל יצור שיש לו שניים או שלושה שכנים, ממשיך לחיות לדור הבא.
3. כל יצור שיש לו יותר משלושה שכנים, מת מצפיפות.
4. כל אתר ריק שיש לו בדיוק שלושה שכנים, קם לתחייה (נולד בו יצור).

מידע נוסף בוויקיפדיה: [Conway's Game of Life](https://en.wikipedia.org/wiki/Conway%27s_Game_of_Life)

Glider  
![Game of Life - Glider](/assets/img/Chapter10/Game_of_life_animated_glider.gif)

Blinker  
![Game of Life - Blinker](/assets/img/Chapter10/Game_of_life_blinker.gif)

Toad  
![Game of Life - Toad](/assets/img/Chapter10/Game_of_life_toad.gif)

</details>



להמשך תרגול כנסו למערכת ההגשות בכיתה שלכם.

[תרגול איומי פרש](/cs/Chapter3Ex3.3/)