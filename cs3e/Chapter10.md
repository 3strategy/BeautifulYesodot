...
int[,] cars = new Car[5, 55]; // (וגם בזה לא היינו משתמשים במציאות) parking ולא 
```
...
א. `for(i=0; i<5; i++) Console.Write(m[2,i]);` -> ידפיס את כל **שורה 2**: 11, 12, 13, 14, 15.

ב. `for(i=0; i<5; i++) Console.Write(m[i,2]);` -> ידפיס את כל **עמודה 2**: 3, 8, 13, 18, 23.

ג. `Console.Write(m);` -> ידפיס את שם הטיפוס (System.Int32[,])

ג. `Console.Write(0,5);` -> ==שגיאה!== אינדקס מחוץ לטווח (האינדקס המקסימלי הוא 4).

ד. `for(i=0; i<5; i++) Console.Write(m[i,i]);` -> ידפיס את **האלכסון הראשי**: 1, 7, 13, 19, 25.
