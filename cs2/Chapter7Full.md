...
- אין כאן מבנה חדש לחלוטין - כבר השתמשנו בפונקציות שקיימות בספריות השפה כגון, `()Console.WriteLine` או `()Math.Sqrt`, וגם ראינו ש-`Main` היא פונקציה מיוחדת שבה מתחילה התוכנית. 
...
- יתרה מכך, פונקציות **מאפשרות לנו להשתמש בפתרון לבעיות שכבר פתרנו**, כדי לפתור בעיות חדשות. כדי להבין עד כמה חזקה הנקודה האחרונה ממולץ לצפות בפרופרסור הרשקוביץ [מסביר על ההבדל בין מתמטיקאי לפיזיקאי](https://youtu.be/P5nstHk0aaY?t=1646) 
...
## הגדרת פונקציה ב-#C:
בתחביר של C#, הגדרת פונקציה (פעולה, מתודה, שיטה, פרוצדורה) נעשית בתוך מחלקה (`class`). עד שנלמד תכנות מונחה-עצמים, נכתוב פונקציות מסוג סטטי (`static`) בתוך המחלקה הראשית של התוכנית, לצד הפונקציה `Main`. התחביר הבסיסי הוא:
...
- הפונקציה **מחזירה בד"כ ערך אחד**. בשלב מתקדם ניתן להראות כיצד להחזיר `tuple`: מקבץ של מספר ערכים.
...
1. הטיפול בערך המוחזרה כעת, "HELLO, WORLD" ממשיך לכיוון ימין כפי שקוראים אנגלית. הסימן נקודה משמש לשרשור פעולות.
1. הפעולה `Replace` פועלת על העצם שהוחזר. היא מקבלת שני פרמטרים: 1. מה מחפשים. 2. מה לשים במקום.
1. התוצאה - הערך המוחזר היא `HELLO, C#`.
