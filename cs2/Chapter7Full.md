...
**הגדרה**: פונקציה היא מקבץ של פקודות המאוגדות תחת **שם מזוהה**, שניתן לקרוא לו (להפעיל אותו) מתוך חלקים שונים בתוכנית, כדי לבצע **משימה מוגדרת**. פונקציה יכולה לקבל נתונים (פרמטרים) ויכולה גם להחזיר תוצאה לפונקציה שקראה לה. **כבר השתמשנו בפונקציות** שקיימות בספריות השפה כגון, `()Console.WriteLine` או `()Math.Round`, וגם ראינו ש-`Main` היא פונקציה מיוחדת, שממנה מתחיל הכל. 
{: .box-success}
...
- פונקציות **מאפשרות לנו להישען על פתרון קודם**, כדי לפתור בעיות חדשות. כדי להפנים **עד כמה זה חשוב**, מומלץ לצפות בפרופרסור הרשקוביץ [כשהוא מתבדח על ההבדל בין מתמטיקאי לפיזיקאי](https://youtu.be/P5nstHk0aaY?t=1646){:target="_blank"}.
</div>
...
## כותרת פונקציה: {#id7syntax}
בתחביר של C# ו-Java, כתיבת פונקציה נעשית בתוך מחלקה (`class`). עד שנלמד תכנות מונחה-עצמים, נכתוב פונקציות מסוג סטטי (`static`) בתוך המחלקה הראשית של התוכנית, לצד הפונקציה `Main`. **התחביר הוא:**
...
- **נקפיד** להוסיף את המילה `static` (כמו בדוגמאות). כשנלמד עצמים נבין מה זה (וזה אחד המושגים שהכי קשה ללמוד).
- הפונקציה **מחזירה בדרך כלל ערך אחד**. בשלב מתקדם נראה כיצד להחזיר `tuple`: מקבץ של מספר ערכים.
...
ניתן **לקרוא לפונקציה** זו מכל פונקציה אחרת (בתוך המחלקה, או במחלקות אחרות) (למשל מתוך Main) **על-ידי כתיבת שמה**, אחריו **סוגריים ריקים** וכמובן **;**

<details open markdown="1"><summary>פתרון עם פונקציה</summary>

{% highlight csharp linenos %}public static void Print10Stars()
{
    for (int i = 0; i < 10; i++)
        Console.Write("*");
...
##  7.2 העברת פרמטרים לפונקציה {#id7.2}
 ...
 **בפתרון שאלות, כולל [בבגרות](https://meyda.education.gov.il/sheeloney_bagrut/2020/6/HEB/899381.pdf?csrt=9702493311115134909), אין התלבטות:** באיזו תצורה של פונקציה להשתמש: מניסוח השאלה **ניתן ונדרש להסיק באופן חד משמעי מהי הגדרת הפונקציה**.  לעיתים מקבלים את ההגדרה המלאה בצורה מפורשת. אך בכל אופן, זו מיומנות שחייבים לרכוש.
![היסק הגדרת פונקציה מניסוח שאלה](image.png)
...
{: .page-break-before}
## 7.3 העברה וקבלת ערכים מהפונקציה {#id7.3}
