...
author: גיא סידס
tags: [ארגומנטים, תת משימות, שיטות, מתודות, פונקציות, סברוטינות, שגרות, אנימציה, העברת פרמטרים, פעולות]
mathjax: true
lang: he
---
...
<details markdown="1"><summary>מינוחים וסדר הוראה</summary>
...
- יתרה מכך, פונקציות **מאפשרות לנו להישען על פתרון קודם שלנו**, כדי לפתור בעיות חדשות. כדי להבין עד כמה חזקה הנקודה האחרונה, ממולץ לצפות בבדיחה של פרופרסור הרשקוביץ [כשהוא מסביר על ההבדל בין מתמטיקאי לפיזיקאי](https://youtu.be/P5nstHk0aaY?t=1646) בקורס מד"ר 1 בטכניון.
</div>
...
\overbrace{\text{static}}^{\text{modifier}}
\quad
\overbrace{\text{ bool }}^{\text{מחזירה}}
\quad
\overbrace{\text{What}}^{\text{שם}}
\quad
...
\quad
\overbrace{\text{Top}}^{\text{שם}}
\quad
\overbrace{\bigl(\text{int}\underbrace{ n=7}_{\text{ברירת מחדל}}\bigr)}^{\text{פרמטרים}}
$$
...
- פונקציות מקבלות פרמטרים, כולל ערכי ברירת מחדל (`int n = 0`).
- ערך החזרה אינו זורם שמאלה/ימינה, אלא מוחזר ישירות ומיושם במקום הקריאה.
- בשרשור שיטות (כדוגמת `s.ToUpper().Replace(...)`), ההחזרה נעשית **דרך נקודה, ימינה**, וכל פעולה מחזירה מצביעה לאובייקט (קיים או חדש).
{: .box-success}
