...
```
כעת, בקריאה לפונקציה יש לספק שני ארגומנטים מתאימים, למשל `PrintSum(5, 7)` ידפיס את השורה $$5 + 7 = 12$$. שימו לב שסדר הארגומנטים חייב להתאים לסדר הפרמטרים כפי שהוגדרו. טיפוס כל ארגומנט נבדק בזמן הקומפילציה - אם ננסה להעביר ערך מטיפוס לא תואם, נקבל שגיאת קומפילציה. (החריג הוא מקרה שבו קיים implicit conversion כמו למשל אם נרצה לשלוח `int` כשמצפים ל-`double` אבל לא להיפך).
...
- אני ממליץ להקפיד לפחות בחלק מהמקרים להשתמש באותיות שונות ב-`Main` לארגומנט ובפונקציה לפרמטר, כדי להמחיש את הנתק בינהם ברגע העברת הארגומנט. ראו בהמשך הערה בנושא **העברת ערכים**.
...
- קיימת גם אפשרות להגדרת ערכי ברירת מחדל לפרמטרים (Default Parameters) כדי להפוך חלק מהם לאופציונליים - נושא זה נדון בנפרד בסוף הפרק.

<details markdown="1">
<summary markdown="1">הרחבה – העברה לפי הפניה **ref**</summary>
...
מדובר בכלי עוצמתי, אך לעיתים מסוכן, ולכן נהוג להשתמש בו רק במקרים חריגים ומוצדקים. שני השותפים למהלך - הן השולח, והן המקבל משתמשים במילה `ref`. כך לפחות יש שקיפות מלאה בנוגע למה שעלול לקרות.
...
<details open markdown="1"><summary>שאלה: לאיזה כיוון הפונקציה מחזירה את הערך (התוצאה)?</summary>
...
עד כאן הכל טוב. אז למה אני טוען שזה שגוי?

בעצם אין כיוון להחזרה, ואם בכלל, צריך לשים לב שיש קדימות דווקה לכיוון ימין. נסתכל לדוגמא על הכתיב המשונה הבא שהוא טיפוסי לעבודה עם עצמים. נשתמש כאן במחרוזת ונטפל בה. הערך שמחזירה הפונקציה אינו חוזר ימינה או שמאלה, הוא פשוט נמצא שם, **ומשתמשים בו לכיוון ימין דווקא על ידי שימוש בנקודה** כמו בדוגמא הבאה:
