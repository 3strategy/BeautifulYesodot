...
AppDbContext ממשיך לנהל חיות ומטפלים ב־App.db. AuthDbContext מנהל חשבונות ב־Accounts.db. בדקו ששני הקבצים נמצאים בתיקיית הפרויקט. מעכשיו מציינים `--context` בכל פקודת EF.

## 2. משתמשים בתשתית Identity

UserManager יוצר משתמש ושומר גיבוב סיסמה. SignInManager בודק סיסמה ומנהל עוגיית כניסה. לא כותבים אלגוריתם הצפנת סיסמאות ולא שומרים סיסמה בעצמנו. לתרגול המקומי אין דרישת אישור דוא״ל.

## 3. בודקים מצבים שונים

אחרי כניסה התפריט מציג את שם החשבון ואת כפתור היציאה. הכניסה מזהה את המשתמש; ההחלטה אילו רשומות מותר לו לערוך תתווסף בשיעור ההרשאות. יציאה מתבצעת ב־POST. כתובת ReturnUrl נבדקת כדי לא להפנות לאתר חיצוני.
