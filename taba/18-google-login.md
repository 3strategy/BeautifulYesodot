...
{: .box-note}
הרחבת רשות בלבד. כניסה מקומית נשארת זמינה גם אם תלמיד לא השלים הגדרות Google. התשתית החיצונית משתמשת בזרימת Identity המוכנה.
...
AddDefaultIdentity כבר מספקת דפי תשתית באזור Identity. הכפתור שולח POST ל־ExternalLogin המוכן; לא כותבים בעצמנו טיפול בטוקנים. במסלול כניסה ראשון ייתכן מסך אישור מובנה באנגלית. זו תשתית הרחבה, לא דרישת עיצוב של הליבה. בלי שני ערכי ההגדרה הכפתור מוסתר והכניסה המקומית פועלת כרגיל.

## 3. גבול האימות שבוצע

במאגר נבדקות הופעת הכפתור והפניית OAuth באמצעות ערכי דמה וללא התחברות לשירות. השלמת כניסה אמיתית דורשת את חשבון Google והסודות שלכם. אין להציג בדיקת הפניה כהוכחה שהכניסה החיצונית כולה הצליחה.
...
תעדו אילו הגדרות נדרשות בכל מחשב, בלי לצלם או להדביק את הסוד. תלמיד שלא השלים את ההרחבה ממשיך להשתמש בכניסה המקומית.
