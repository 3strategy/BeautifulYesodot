...
## טבלת מעקב ראשונה עם תנאי

כאשר מופיע תנאי בקוד, בטבלת המעקב נרשום גם את תוצאת הביטוי הלוגי: `true` או `false`.
בשלב הראשון נמספר את שורות הקוד, ובטור הראשון בטבלת המעקב נכתוב את מספר השורה שבוצעה.
אם ערך של משתנה לא השתנה בשורה מסוימת, נשאיר את התא שלו ריק.

נעקוב אחר הקוד עבור הערך ההתחלתי `score = 72`:
...
| שורה | `score` | `passed` | `score >= 60` | פלט |
| ---- | ------- | -------- | ------------- | --- |
| 1    | 72      |          |               |     |
| 2    |         | false    |               |     |
| 4    |         |          | **true**      |     |
...
שימו לב: שורות 5 ו-7 הן סוגריים בלבד, ולכן בדרך כלל אין צורך להוסיף עבורן שורה בטבלת המעקב.
